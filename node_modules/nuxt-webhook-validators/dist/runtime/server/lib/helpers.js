...
export const computeSignature = async (secretKey, algorithm, payload, options) => {
...
export const verifyPublicSignature = async (publicKey, algorithm, payload, signature, options) => {
  const encoding = options?.encoding ?? "hex";
...
export const ensureConfiguration = (provider, event) => {
  if (configContext.provider) provider = configContext.provider;
