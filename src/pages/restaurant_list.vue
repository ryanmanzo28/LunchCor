    ...
    <section class="panel" aria-labelledby="restaurants-heading">
      ...
      <div v-if="isMenuModalOpen" class="menu-modal-backdrop" role="presentation" @click="closeMenuModal">
        ...
        >
          ...
          <p v-else-if="menuLoadError" class="muted">{{ menuLoadError }}</p>
          <ul v-else-if="selectedMenuItems.length" class="menu-items-list">
            <li v-for="item in selectedMenuItems" :key="`${item.section ?? 'uncategorized'}-${item.name}`" class="menu-item-row">
              <div>
